...
exports.dashboard = async (req, res) => {
  try {
    ...
    // FILTER meals
    user_data.meal = user_data.meal.filter((food) => food.datetime >= today);
...
exports.dashboardSearch = async (req, res) => {
  try {
    ...
    //   {
    //     $match: {
    //       'meal$.datetime': { $gte: start_date_time, $lt: end_date_time }
    //     }
    //   },
    //   { $project: { meal: 1 } },
    //   { $sort: { sortBy: order } }
    ...
    user_data.meal = user_data.meal.filter(
      (food) =>
        food.datetime >= start_date_time && food.datetime < end_date_time
    );

    // Check if only one day meals
    if (end_date_time - start_date_time === 86400000) user_data.showBar = true;
    ...
    if (req.query && Object.keys(req.query).length > 0) {
      ...
      // SORT meals
      user_data.meal.sort((a, b) => order * a[sortBy] - order * b[sortBy]);
    }
